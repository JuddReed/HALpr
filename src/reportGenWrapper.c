...
void play(string thisPage, char* report, char* uid) {
  ...
  char *value = strchr(wholeForm,'=')+1;	// strip off leading "ReportText=
  if(!strcmp(report,"EvaPlan")) {
    printf(
...
void tell(string thisPage, char* report) {
  ...
  if(strstr(report,"vaPlan")) { // its an eva plan so show us the map
    ...
    "    <li><a href=%s+play+%s+%s>Review Posted Report</a></li>\n",
...
*/
  ...
  printf("  <a href=reportReview%s+days>Review Old Reports</a>\n",strchr(thisPage.c_str(),'?'));
}
...
Session *sess = new Session(argc,argv);
  ...
  else if(!strcmp(argv[2],"days")) { days(sess->getRefreshLink()); }	// menu of reports by day
  else if(!strcmp(argv[2],"play")) { play(sess->getRefreshLink(), argv[3], argv[4]); }// show previously submitted report
